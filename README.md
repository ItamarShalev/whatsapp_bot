...
We validate the environment variables using `zod`, get the schema from `src/env.ts`. See example at `env.example`.

| Variable           | Requiered | Default                        | Description                                                          |
| ------------------ | --------- | ------------------------------ | -------------------------------------------------------------------- |
| ADMIN_PHONE_NUMBER | Yes       |                                | The phone number of the admin in E.164 format.                       |
| LANGUAGE           | No        | en                             | Specify the language for the templates. eg. en, he                   |
| COMMAND_PREFIX     | No        | !                              | The prefix for triggering the commands. eg. !, /, .                  |
...
| ------- | ------------------------------------ | ------------------------ |
| wlecome | Welcome a new user joining the group |                          |
| poll    | Create a poll in the group           | `question`, ...`options` |
| ping    | Ping the bot to check if it's alive  |                          |

## Templates Guide

The templates are pleced in the `templates` folder. Each subfolder is the language code, e.g. `en` for English, `he` for Hebrew. The templates' name should be the same as the command name in cabab case, e.g. `say-hi.md` for the `SayHi` command.

Generate a commend with templates using the commend:
...
- Use the ICU templeting format for variables and pluralization, see the [ICU documentation](https://messageformat.github.io/messageformat/guide/) for more information.

## Commands Guilde

Each command class must be placed in the `src/commands` folder. The command class should extend the `CommandBase` class and have templetes in each language of the `templates` folders.
...
Here are a summary of the available events:
